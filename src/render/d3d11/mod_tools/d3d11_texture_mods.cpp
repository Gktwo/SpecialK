...
SK_D3D11_LiveTextureView(bool& can_scroll, SK_TLS* pTLS = SK_TLS_Bottom())
{
	...
	ImGui::SameLine();
	ImGui::Text(u8"\t当前列表表示的纹理内存 %5.2f MiB ",
		(double)total_texture_memory / (1024.0 * 1024.0));

	ImGui::PushID(u8"Texture2D_D3D11");
	ImGui::BeginGroup();
	...
	if (ImGui::IsItemHovered())
	{
		if (tex_set == 1)
		{
			ImGui::SetTooltip(u8"使用上一帧绘制的纹理刷新列表.");
		}
		else
		{
			ImGui::SetTooltip(u8"使用所有缓存的纹理刷新列表.");
		}
	}
	...
	if (non_mipped > 0)
	{
		...
		if (ImGui::Button(u8"  生成 Mipmaps  ###GenerateMipmaps_ALL"))
		{
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip(u8"目前有%lu的纹理没有mipmaps", non_mipped);
	}

	static float const slen =
		(float)strlen(u8"使用的贴图   ") / 2.0f;

	ImGui::SameLine();
	ImGui::PushItemWidth(font_size * slen);

	ImGui::Combo("###TexturesD3D11_TextureSet", &tex_set, u8"所有的纹理\0使用的纹理\0\0", 2);

	ImGui::PopItemWidth();
	ImGui::SameLine();

	if (ImGui::Button(u8" 清除调试 "))
	{
	...
	if (ImGui::IsItemHovered()) ImGui::SetTooltip(u8"退出纹理调试模式.");

	ImGui::SameLine();

	if (ImGui::Button(u8"  重新加载所有注入的纹理  "))
	{
		SK_D3D11_ReloadAllTextures();
	}

	//ImGui::SameLine ();
	ImGui::Checkbox(u8"在游戏中高亮选中的纹理##D3D11_HighlightSelectedTexture",
		&config.textures.d3d11.highlight_debug);
	ImGui::SameLine();

	static bool hide_inactive = false;

	ImGui::Checkbox(u8"隐藏非活动纹理##D3D11_HideInactiveTextures",
		&hide_inactive);

	ImGui::SameLine();

	ImGui::PushItemWidth(font_size * slen);
	ImGui::SliderInt(u8"帧间列表刷新", &refresh_interval, 0, 120);
	ImGui::PopItemWidth();
	...
	if (ImGui::IsItemHovered() || ImGui::IsItemFocused())
	{
		...
		if (ImGui::IsItemFocused())
		{
			...
			ImGui::Separator();
			ImGui::BulletText(u8"按LB从列表中选择上一个纹理");
			ImGui::BulletText(u8"按RB从这个列表中选择下一个纹理");
			ImGui::EndTooltip();
		}
		...
		else
		{
			...
			ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.2f, 1.0f), "");
			ImGui::Separator();
			ImGui::BulletText(u8"按%hs从列表中选择上一个纹理", SK_WideCharToUTF8(virtKeyCodeToHumanKeyName[VK_OEM_4]).c_str());
			ImGui::BulletText(u8"按%hs从列表中选择下一个纹理", SK_WideCharToUTF8(virtKeyCodeToHumanKeyName[VK_OEM_6]).c_str());
			ImGui::EndTooltip();
		}
...

	{
		...
		if (pDev != nullptr)
		{
			...
			//{
			//  if (ImGui::IsItemHovered ())
			//    ImGui::SetTooltip (u8"Click me to make this texture the visible version.");
			//
			...
			if (pTex != nullptr)
			{
				...
				ImGui::BeginGroup();
				ImGui::TextUnformatted(u8"格式:       ");
				ImGui::TextUnformatted(u8"哈希:         ");
				ImGui::TextUnformatted(u8"数据大小:    ");
				ImGui::TextUnformatted(u8"加载时间:    ");
				ImGui::TextUnformatted(u8"缓存命中率:   ");
				ImGui::TextUnformatted(u8"参考:   ");
				ImGui::EndGroup();
				ImGui::SameLine();
				ImGui::BeginGroup();
				ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
				...
				ImGui::Checkbox(u8"垂直翻转##D3D11_FlipVertical", &flip_vertical);   ImGui::SameLine();
				ImGui::Checkbox(u8"水平翻转##D3D11_FlipHorizontal", &flip_horizontal);

				if (!entry.injected)
				{
					if (!SK_D3D11_IsDumped(entry.crc32c, 0x0))
					{
						if (ImGui::Button(u8"  将纹理转储到磁盘  ###DumpTexture"))
						{
					...
					{
						if (ImGui::Button(u8"  从磁盘中删除转储纹理  ###DumpTexture"))
						{
					...
					if (entry.mipmapped == FALSE)
					{
						...
						if (ImGui::Button(u8"  生成 Mipmaps  ###GenerateMipmaps"))
						{
							SK_ScopedBool decl_tex_scope(
								SK_D3D11_DeclareTexInjectScope(pTLS)
							);
				...
				{
					ImGui::SameLine();
					ImGui::TextColored(ImColor::HSV(0.25f, 1.0f, 1.0f), u8"分级纹理还不能被重新注入.");
				}

				if (SK_D3D11_IsTextureUncacheable(pTex))
				{
					ImGui::SameLine();
					ImGui::TextColored(ImColor::HSV(0.05f, 1.0f, 1.0f), u8"纹理不能被重新注入.");
				}

				if (entry.injected)
				{
					if (ImGui::Button(u8" 重载纹理  ###ReloadTexture"))
					{
						SK_D3D11_ReloadTexture(pTex);
					}

					ImGui::SameLine();
					ImGui::TextColored(ImVec4(0.05f, 0.95f, 0.95f, 1.0f), u8"这种纹理被注入到原来的纹理上.");
				}
					...
					effective_width != (float)(tex_desc.Width >> lod))
				{
					...
					ImGui::TextColored(ImColor::HSV(0.5f, 1.0f, 1.0f), u8"纹理被重新缩放以适应.");
				}
				...
				if (SUCCEEDED(pDev->CreateShaderResourceView(pTex, &srv_desc, &pSRV.p)))
				{
					...
					if (ImGui::IsItemHovered())
					{
						if (SK::ControlPanel::current_time - dwLastUnhovered > 666UL)
						{
							...
							ImGui::Text(u8"%hs", SK_D3D11_DescribeUsage(
								tex_desc.Usage));
							ImGui::Text(u8"%hs", SK_D3D11_DescribeBindFlags(
								(D3D11_BIND_FLAG)tex_desc.BindFlags).c_str());
							if (tex_desc.MiscFlags != 0)
							{
								ImGui::Text(u8"%hs", SK_D3D11_DescribeMiscFlags(
